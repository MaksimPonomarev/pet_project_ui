...
from pages.locators import CartPageLocators, CartItemLocators
...
class CartPage(BasePage):
    ...
    def check_quantity(self, item_id: str, expect_quantity: str):
    ...
    def should_be_added_products(self, cart_items: dict[str, CartItem]):
        ...
        for id_product, product_info in cart_items.items():
            ...
            self.assert_equal(product_info.qty, int(product_quantity))
            self.assert_equal(self.get_total_price(product_price, product_quantity), self._parse_price(product_total))

    def should_not_be_visible_elem_by_id(self, product_id: str):
